...
def densenet121(pretrained=False, **kwargs):
    ...
    """
    model = DenseNetEfficient(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    if pretrained:
...
class _EfficientDensenetBottleneck(nn.Module):
    ...
    is not permanent, these features are recomputed during the backward pass.
    """
...
class DenseNetEfficient(nn.Module):
    ...
    """
    def __init__(self, growth_rate=12, block_config=(6, 12, 24, 16), compression=1,
                 num_init_features=64, bn_size=4, drop_rate=0,
                 num_classes=1000, small_inputs=False):
